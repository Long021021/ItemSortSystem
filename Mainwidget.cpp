...
void Widget::layoutInit()
{
    ...
    inferThread = new QThread(this);
    infer = new RK3568Infer("/userdata/yolov5s-640-640.rknn");
    // cameraInfoThread = new QThread;
    // camera = new Camera;

    infer->moveToThread(inferThread);
    ...
    connect(inferThread, &QThread::finished, inferThread, &QObject::deleteLater);
    connect(inferThread, &QThread::started, infer, &RK3568Infer::startProcess);
    // connect(camera, &Camera::frameIsReady, infer, &RK3568Infer::processFrame);
    connect(infer, &RK3568Infer::inferenceFinished, this, [=](const QImage &img){
        if(!img.isNull() && video_display){
            video_display->setPixmap(QPixmap::fromImage(img.scaled(
                video_display->size(), Qt::KeepAspectRatio)));
        }
    });
